...
using UnityEngine;
using GHToolkit.Components;

//Source: https://gist.github.com/sanyae2439/dbb0b4b439ad4a2a0f6c42d68e2c82dc#file-syncvarhackextensions-cs
...
namespace GHToolkit
{
    public static class Extensions
    {
        ...
        private static readonly Dictionary<Player, ExtraClassComponent> players = new Dictionary<Player, ExtraClassComponent>();

        public static bool IsExtraClass(this Player player,string name)
        {
            return player.HasExtraClass() && player.GetExtraClass().Id == Util.GetIdForAlias(name);
        }

        public static void RemoveExtraClass(this Player player)
        {
            if (player.HasExtraClass())
            {
                UnityEngine.Object.Destroy(player.GetExtraClass());
                players.Remove(player);
            }
        }

        public static void AssignExtraClass(this Player player, Type clazz)
        {
            if (!clazz.IsSubclassOf(typeof(Components.ExtraClassComponent)))
            {
                throw new ArgumentException("Passed class is not inherited from ExtraClassComponent");
            }
            player.RemoveExtraClass();
            players.Add(player, (ExtraClassComponent)player.GameObject.AddComponent(clazz));
        }

        public static bool HasExtraClass(this Player player)
        {
            return players.ContainsKey(player);
        }

        public static Components.ExtraClassComponent GetExtraClass(this Player player)
        {
            return player.HasExtraClass() ? players[player] : null;
        }
